...
function initParseContext(cxt: ParseContext) {
    cxt.init(defaultStore, {
...
function checkArgumentLength(node: ModifierNode, n: number) {
    ...
    name: string, argNames: string[], slotName: string, content: _Ent<T>[])
{
    ...
    mod.beforeProcessExpansion = (node: ModifierNode, cxt: ParseContext) => {
        if (!node.state?.ok) return [];
        const store = cxt.get(defaultStore)!;
...
function slotModifier<T extends 'inline' | 'block'>(type: T): _Def<T, any> {
    ...
    mod.prepareExpand = (node: ModifierNode, cxt: ParseContext, immediate: boolean) => {
        ...
        const stack = 
            ({ inline: store.inlineSlotDelayedStack, block: store.blockSlotDelayedStack })[type];
